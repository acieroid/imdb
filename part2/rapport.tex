...
select FirstName, LastName, Num, count(*) as compteur
  ...
  \item R6 :
    % TODO
    \begin{lstlisting}
select count(Episode.ID), avg(EpisodesPerSeason.Count), avg(ActorsPerSeason.Count)
  from Episode, Serie, Work,
       (select Episode.SID as ID, count(*) as Count
          from Episode group by Season) EpisodesPerSeason,
       (select Episode.SID as ID, count(*) as Count
          from Actor, Episode
          where Actor.ID = Episode.ID
          group by Episode.Season) ActorsPerSeason,
