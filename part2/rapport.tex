...
\begin{list}{-}{}
  ...
  \item R3 :
    ...
    \begin{align*}
      ...
      C & \leftarrow Movie * \pi_{ID}_(Actor * Y2) \\
...
select distinct FirstName, LastName, Num
  ...
  \item R3 :
    Si les infos sur l'acteur X sont dans $XFName$, $XLName$, $XNum$
% TODO: vérifier que ça marche dans sqlite, ça n'a pas l'air de
% retourner les bons résultats ici
    \begin{lstlisting}
...
select ID from Episode
  where not exists (select * from Person, Actor
    ...
    \end{lstlisting}
  \item R5 :
%% select FirstName, LastName, Num, count(*) as Count
%%   from Actor, Serie
